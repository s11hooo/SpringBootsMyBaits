...
public class UserInfoController {
    ...
    public UserInfoDTO getUserExists(HttpServletRequest request) throws Exception {
        ...
        String userId = CmmUtill.nvl(request.getParameter("userId")); // 회원아이디
    ...
    public UserInfoDTO getEmailExists(HttpServletRequest request) throws Exception {
        ...
        String email = CmmUtill.nvl(request.getParameter("email")); // 회원이메일
    ...
    public MsgDTO insertUserInfo(HttpServletRequest request) {
        ...
        try {
             ...
             * ######################################################################
             */
            String userId = CmmUtill.nvl(request.getParameter("userId")); // 아이디
            String userName = CmmUtill.nvl(request.getParameter("userName")); // 이름
            String password = CmmUtill.nvl(request.getParameter("password")); // 비밀번호
            String email = CmmUtill.nvl(request.getParameter("email")); // 이메일
            String addr1 = CmmUtill.nvl(request.getParameter("addr1")); // 주소
            String addr2 = CmmUtill.nvl(request.getParameter("addr2")); // 상세주소
            /*
             * ######################################################################
    ...
    public MsgDTO loginProc(HttpServletRequest request, HttpSession session) {
        ...
        try {
             ...
             */
            if (!CmmUtill.nvl(rDTO.getUserId()).isEmpty()) { //로그인 성공
                ...
                session.setAttribute("SS_USER_NAME", CmmUtill.nvl(rDTO.getUserName()));
    ...
    public String searchUserIdProc(HttpServletRequest request, ModelMap model) throws Exception {
        ...
        UserInfoDTO rDTO = (UserInfoDTO) Optional.ofNullable(UserInfoService.searchUserIdOrPasswordProc(pDTO))
    ...
    public String searchPasswordProc(HttpServletRequest request, ModelMap model, HttpSession session) throws Exception {
        ...
        // 비밀번호 찾기 가능한지 확인하기
        UserInfoDTO rDTO = (UserInfoDTO) Optional.ofNullable(UserInfoService.searchUserIdOrPasswordProc(pDTO)).orElseGet(UserInfoDTO::new);
    ...
    public String newPasswordProc(HttpServletRequest request, ModelMap model, HttpSession session) throws Exception {
        ...
        // 정상적인 접근인지 체크
        String newPassword = CmmUtill.nvl((String) session.getAttribute("NEW_PASSWORD"));
        ...
        if (!newPassword.isEmpty()) { //정상 접근
            ...
            String password = CmmUtill.nvl(request.getParameter("password")); //신규 비밀번호
