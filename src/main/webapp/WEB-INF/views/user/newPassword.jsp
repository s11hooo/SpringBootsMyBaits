...
<%@ page import="kopo.poly2.dto.UserInfoDTO" %>
...
<%
    ...
    if (CmmUtill.nvl(rDTO.getUserId()).length() > 0) {
...
<head>
    <meta charset="UTF-8">
    <title><%=CmmUtill.nvl(rDTO.getUserName())%> 회원님의 비밀번호 재설정</title>
...
<h2><%=CmmUtill.nvl(rDTO.getUserName())%> 회원님의 비밀번호 재설정</h2>
